...
const prefix = "u!"
const developers = "742110454481944599"
const activities = [
  "Soviet Union - SystemBot",
  "Use u!help",
  "We Are Legion !",
  "We Are Controllers"
];
...
client.on("message", async message => {
  ...
  if (message.guild.id != `871326250264854598`) return;
  if (message.channel.id != `916021908716281916`) return;
...
client.on('message', message => {
  if(message.content.startsWith(prefix + "help")) {
    ...
    .setColor("#990101")
    .setTitle(`\`SovietUnion - u!help\``)
    .addField('**🌍 General Commands** ',`\`u!profile\` - \`u!user\` - \`u!bot\` - \`u!ping\` - \`u!server\` - \`u!avatarserver\` - \`u!tax\` - \`u!id\` - \`u!daily\` - \`u!credit\` - \`u!uptime\` - \`u!bans\` - \`u!feedbacks\` - \`u!report\` - \`u!sug\` - \`u!anime\` - \`u!guilds\` - \`u!boosts\` - \`u!bots\` - \`u!say\` - \`u!rules\` - \`u!discrim\` - \`u!top\` - \`u!top text\` - \`u!top voice\` - \`u!apply\` - \`u!help\``)
    .addField('**🔧 Admin Commands**',`\`u!hide\` - \`u!show\` - \`u!lock\` - \`u!unlock\` - \`u!unban\` - \`u!ban\` - \`u!unmute\` - \`u!mute\` - \`u!vmute\` - \`u!vunmute\` -\`u!vkick\` - \`u!role\` - \`u!warnv\` - \`u!server\` - \`u!clear\` - \`u!cv\` - \`u!cc\` - \`u!cc category\` - \`u!setnick\` - \`u!short\` - \`u!room1\``)
    .addField('🎮 Fun Commands',`\`u!triggered\` - \`u!trash\` - \`u!gay\` , \`u!hug\` - \`u!slap\` - \`u!wanted\` - \`u!meme\` - \`u!nokta\` - \`u!sara7a\` - \`u!za7f\` - \`u!winer\` - \`u!game\` - \`u!spank\` - \`u!kill\` - \`u!cut\` - \`u!blur\` - \`u!circle\` - \`u!rip\` - \`u!douknow\` - \`u!astkhfr\``)
      .addField('**💵 TicketSupport Commands**',`\`u!new\` - \`u!close\` - \`u!delete\` - \`u!add\` - \`u!remove\` - \`u!rename\` - \`u!createlog\``)
      .addField('🛡️ Protection Commands ',`\`u!antibots on\` - \`u!antibots off\` - \`u!antilinks on\` - \`u!antilinks off\` - \`u!antihack on\` - \`u!antihack off\``)
      .addField('👥 Invites Commands', `\`u!invite\` - \`u!topinvites\` - \`u!info\``)
       .addField('📪 Broadcast Commands', `\`u!ebc\` - \`u!obc\` - \`u!bc\``)
       .addField('🏙️  Room Commands', `\`u!cc report\` - \`u!cc feedback\` - \`u!cc suggestions\` - \`u!cc welcome\``)
      .setImage('https://cdn.discordapp.com/attachments/913819000159477761/916055793672028190/gethelp1.png')
...
client.on('message', message => {
    if (message.content.startsWith(prefix + "profile")) {
      ...
      message.guild.fetchInvites().then(invites => {
        ...
        .setFooter(`${prefix}help - Welcome To Soviet Union`,client.user.avatarURL())
...
client.on("message", russi => {
  if (russi.content === prefix + "server") {
    ...
    .setColor('#990101')///Russi
    .setFooter(`Requested | ${russi.author.tag}`, russi.author.avatarURL())
    .addField('> <:id:791203015708770304> Server ID :', `${russi.guild.id}`)
...
client.on('message', badboy => {
  if(badboy.content.startsWith(prefix + "feedback")){
    ...
    var fedbackchannel = badboy.guild.channels.cache.find(channel => channel.name === "feedbacks"); //اسم الروم
...
client.on('message', badboy => {
  if(badboy.content.startsWith(prefix + "sugg")){
    const args = badboy.content.split(" ").slice(1).join(" ")
    if(!args) return badboy.channel.send("اكتب اقتراح")
    var fedbackchannel = badboy.guild.channels.cache.find(channel => channel.name === "suggestions"); //اسم الروم
...
3-ممنوع نشر الروابط
4-احترام الأعضاء 
5-بدك اي مساعدة كلم اي واحد عنده رانك إدارة في الخاص او الشات العام أو افتح تكت الدعم الفني
6-لا تشتري من احد إلا اللي يكون عنده رانك بائع
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "تقديم")) {
    if (!message.channel.guild) return;
    if (message.author.bot) return;
    let channel = message.guild.channels.cache.find(gg => gg.name === "இ・التقديمات");
    if (!channel)
